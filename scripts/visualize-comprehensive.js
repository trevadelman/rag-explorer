...
/**
 * RAG Comprehensive Benchmark Visualization Tool
 * 
 * This script generates visualizations from comprehensive benchmark results
 * to help analyze and compare different RAG configurations.
 */
...
// Default input file
const DEFAULT_INPUT_FILE = 'comprehensive-benchmark.json';
...
async function main() {
  try {
    console.log(`Reading comprehensive benchmark results from ${inputFile}...`);
    ...
    // Save HTML report
    const outputFileName = inputFile.replace('.json', '-report.html');
...
function generateHtmlReport(benchmarkResults) {
  const { results, timestamp, config } = benchmarkResults;
  ...
  if (successfulResults.length === 0) {
    return `
      ...
      <html>
        <head>
          <title>RAG Comprehensive Benchmark Results</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
          </style>
        </head>
        <body>
          <h1>RAG Comprehensive Benchmark Results</h1>
            ...
            <table>
              ...
              ${successfulResults
                ...
                <tr>
                  ...
                  <td>${result.metrics.keywordMatchPercentage.toFixed(2)} (${keywordsMatched}/${expectedKeywords.length})</td>
                </tr>
              `}).join('')}
            </table>
          </div>
