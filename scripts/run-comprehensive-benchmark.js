 ...
 * - Embedding models (OpenAI small/large, Gemini)
 * - Content types (xeto, markdown, documentation)
 * 
 * And saves all results to a single file for easy comparison.
 */
...
// Configuration
const CONFIG = {
  // Search strategies to benchmark
  searchStrategies: [
    'vector-search',
    'hybrid-search',
    'combined-search'
  ...
  outputFile: `results/benchmark-${new Date().toISOString().replace(/:/g, '-')}.json`
};
...
async function runComprehensiveBenchmarks() {
  console.log(chalk.bold.blue('🔍 Comprehensive RAG Benchmarking'));
  console.log(chalk.blue('================================'));
  console.log(chalk.cyan(`Search Strategies: ${chalk.white(CONFIG.searchStrategies.join(', '))}`));
  ...
  const allResults = [];
  const totalCombinations = CONFIG.searchStrategies.length * CONFIG.llmModels.length * CONFIG.embeddingModels.length * CONFIG.contentTypes.length * CONFIG.contextSizes.length;
  let completedCombinations = 0;
  
  // Run benchmarks for each combination
  for (const searchStrategy of CONFIG.searchStrategies) {
      ...
      for (const embeddingModel of CONFIG.embeddingModels) {
        for (const contentType of CONFIG.contentTypes) {
          for (const contextSize of CONFIG.contextSizes) {
            ...
            console.log(chalk.cyan(`   Strategy: ${chalk.white(searchStrategy)}`));
          ...
          try {
            ...
            const results = await runBenchmark({
              ...
              outputFile: `temp-${searchStrategy}-${llmModel}-${embeddingModel}-${contentType}-${contextSize.name}.json`
            });
  ...
  try {
    ...
    const tempFiles = files.filter(file => file.startsWith('temp-') && file.endsWith('.json'));
    
    for (const file of tempFiles) {
      await fs.unlink(file);
      console.log(chalk.gray(`  Deleted: ${file}`));
    }
    console.log(chalk.green(`  Cleaned up ${tempFiles.length} temporary files`));
  } catch (error) {
    console.error(chalk.yellow(`  Warning: Could not clean up temporary files: ${error.message}`));
  }
...
function generateSummary(results) {
  ...
  for (const [model, modelResults] of Object.entries(byEmbedding)) {
    const avgTime = average(modelResults.map(r => r.metrics.vectorSearchTime));
    const avgCost = average(modelResults.map(r => r.metrics.embeddingCost));
    
    console.log(chalk.yellow(`  ${model}:`));
    console.log(chalk.white(`    Avg Vector Search Time: ${avgTime.toFixed(2)}ms`));
    console.log(chalk.white(`    Avg Cost: $${avgCost.toFixed(6)}`));
  }
